...
public:
  ...
  Combinatorics() {
    // Предвычисляем биномиальные коэффициенты C(n,k)
    for (int n = 0; n <= N; n++) {
      for (int k = 0; k <= K; k++) {
...
class RubiksCube {
private:
  // Позиции и ориентации ребер и углов


  // Индексы для ребер и углов
  enum EdgePositions {
...
public:
  ...
  array<uint8_t, 12> edge_orientations;
  array<uint8_t, 8> corners;
  array<uint8_t, 8> corner_orientations;
  RubiksCube() {
    // Исходное собранное состояние
...
private:
  ...
  // Вспомогательные методы для поворотов
...
public:
  ...
  bool in_phase1() const {
    ...
    // Проверка middle-ребер (они должны быть на своих местах)
  ...
  // 1. Хеш ориентаций рёбер (12 бит, 0-4095)
  ...
  // 2. Хеш ориентаций углов (7 элементов, 0-6560)
  ...
  // 6. Хеш позиций средних рёбер (0-23)
...
// =============================================
// Класс для реализации алгоритма Коцембы
// =============================================
...
private:
  ...
  // Генерация таблицы предвычислений
  template<size_t N>
  ...
  // Поиск с итеративным углублением для фазы 1
  ...
  // Поиск с итеративным углублением для фазы 2
...
public:
  ...
  vector<Move> solve(RubiksCube cube) {
    ...
    // Проверяем, что фаза 1 действительно завершена
    if (!cube.in_phase1()) {
    ...
    // Оптимизация решения (удаление избыточных ходов)
...
private:
  // Оптимизация последовательности ходов
  void optimize_solution(vector<Move>& solution) {
    ...
    for (size_t i = 1; i < solution.size(); i++) {
      ...
      // Проверяем, можно ли объединить ходы
...
private:
  ...
  // Расставил цвета в особом порядке, первый цвет ребра всегда
  // был  на позиции в приоритете 1) U/D 2) L/R, для углов первый цвет на U/D, второй
  // цвет и третий цвета слева направо для верха и наоборот для низа
  ...
  // Функция для создания мапы для рёберных позиций
  ...
  // Функция для создания мапы для угловых позиций
...
public:
  void Parse(RubiksCube& cube, const string& state) { 
    ...
    int idx = 0;
    // Если UD цвет на FB гранях, то флип
    // Инача если цвет на гранях UD LR - UD LR, то не флипнут, иначе флипнут 
    ...
    idx = 0;
    // Я расставил цвета так, что c1 - UD, с2 - лево для верха/ право для низа, c3 - оставшийся
...
extern "C" {
  // Создаёт объект Solver в куче и возвращает указатель
  Solver* Solver_new() {
    return new Solver();
  }
  ...
  // Решает кубик Рубика и записывает решение в буфер
  void Solver_solve(Solver* solver, const char* state, char* solution_buffer, int buffer_size) {

    // Вызываем метод Solve, передавая временный буфер
    solver->Solve(state, solution_buffer); // solution_buffer будет изменён внутри

    // Если решение не помещается в буфер, обрезаем его
    if (strlen(solution_buffer) >= buffer_size - 1) {
      solution_buffer[buffer_size - 1] = '\0'; // Гарантируем null-terminated строку
    }
  }
}
...
int main() {
  CubeParser parser;
  RubiksCube cube;
  //string state = "BUBLULBRULDLLFBUBRFDLRRUUURURRFBDDDFDFDFLUDRFRLBFDBLBF";
  string state = "FRDDUDRDFUBDFFUBRURRBBRLFULRBRDBBFUDUFBLLUBLLUFLFDLLRD";
  parser.Parse(cube, state);
  KociembaSolver solver;
  vector<Move> solution = solver.solve(cube);
  ...
  if (cube.is_solved()) {
    cout << "Cube solved successfully!" << endl;
  }
  else {
    cout << "Solution failed to solve the cube." << endl;
  }
